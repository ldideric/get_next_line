...
/*   Updated: 2020/01/27 12:45:07 by ldideric      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static char	*eachline(char **buffer, char *str, int ret)
{
	...
	if (*buffer[0] == '\0')
	{
		new = ft_strdup(str, TRUE);
		free(str);
		if (new == NULL)
			return (NULL);
		if (ret == 0 && new[ft_strlen(new, TRUE) - 1] != '\n')
		{
			new[ft_strlen(new, FALSE)] = '\n';
			new[ft_strlen(new, FALSE)] = '\0';
		}
		return (new);
	}
...
{
	while (filed->str[ft_strlen(filed->str, TRUE) - 1] != '\n')
	{
	...
	filed->str[ft_strlen(filed->str, TRUE) - 1] = '\0';
	return (filed->str);
}
...
int			get_next_line(int fd, char **line)
{
	...
	if (filed[fd].buffer == NULL || filed[fd].buffer[0] == '\0')
	{
		...
		filed[fd].buffer = ft_calloc(sizeof(char) * (BUFFER_SIZE + 1));
